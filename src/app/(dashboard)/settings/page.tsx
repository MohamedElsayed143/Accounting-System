...
import { UserPlus, Shield, Trash2, Lock, User as UserIcon } from "lucide-react";
...
const DEFAULT: Settings = {
    ...
    roles: {
      worker: {
        ...
        permissions: {
          ...
          reports_ledger: false,
          returns_sales: false,
          returns_purchase: false,
          sales_quotations_view: true,
          sales_pending_view: true,
          settings_access: false,
          accounting_ledger_view: false,
          accounting_journal_view: false,
          accounting_journal_add: false,
          accounting_coa_view: false,
        },
      },
    },
  },
  notifications: {
    ...
    allowWorkersManualJournals: false,
  },
};
...
const PERMISSION_GROUPS = [
  ...
  },
  {
    group: "الخزينة",
    perms: [
      { key: "treasury_view", label: "عرض الخزينة والبنوك" },
      { key: "treasury_manage", label: "إدارة الخزينة والبنوك" },
  ...
  },
  {
    group: "المالية",
    perms: [
      { key: "accounting_ledger_view", label: "كشف الحسابات (دفتر الأستاذ)" },
      { key: "accounting_journal_view", label: "عرض سجل القيود" },
      { key: "accounting_journal_add", label: "إضافة قيد يدوي" },
      { key: "accounting_coa_view", label: "دليل الحسابات (شجرة)" },
    ],
  },
];
  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm overflow-hidden">
      <div className="flex items-center gap-3 px-5 py-4 border-b border-slate-100 dark:border-slate-800 bg-slate-50/70 dark:bg-slate-800/40">
        ...
        <h3 className="font-bold text-slate-800 dark:text-white flex-1">
          {title}
        </h3>
        {badge}
      </div>
...
function Toggle({
  ...
  return (
    <label className="flex items-center justify-between gap-4 cursor-pointer p-3 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
      ...
      >
        <div
          className={`absolute top-1 w-4 h-4 rounded-full bg-white shadow transition-all duration-200 ${checked ? "right-1" : "left-1"}`}
        />
      </div>
    </label>
  );
...
export default function SystemSettingsPage() {
                  ...
                  roles: {
                    worker: {
                      ...
                      permissions: {
                        ...
                        ...(dbSettings.rbac.roles?.worker?.permissions ||
                          dbSettings.rbac.roles?.cashier?.permissions ||
                          {}),
                      },
                    },
                  },
        ...
        if (companySettings) {
          setS((prev) => ({
            ...
            company: {
              ...
              logoBase64:
                companySettings.companyLogo || prev.company.logoBase64,
              stampBase64:
                companySettings.companyStamp || prev.company.stampBase64,
              barcodeBase64:
                (companySettings as any).companyBarcode ||
                prev.company.barcodeBase64,
            },
            invoice: {
              ...prev.invoice,
              ...
              invoiceName:
                companySettings.invoiceName || prev.invoice.invoiceName,
              salesInvoiceName:
                companySettings.salesInvoiceName || "فاتورة مبيعات",
              purchaseInvoiceName:
                companySettings.purchaseInvoiceName || "فاتورة مشتريات",
              ...
              showBarcodeOnPrint:
                (companySettings as any).showBarcodeOnPrint ?? true,
              termsAndConditions:
                companySettings.termsAndConditions ||
                prev.invoice.termsAndConditions,
              invoiceFooterNotes:
                (companySettings as any).invoiceFooterNotes || "",
            },
            tax: {
              ...prev.tax,
            ...
            notifications: notifSettings
              ? {
                  ...
                  requireApprovalForTransfers:
                    notifSettings.requireApprovalForTransfers,
                  requireApprovalForSafeCreation:
                    notifSettings.requireApprovalForSafeCreation,
                  requireApprovalForBankCreation:
                    notifSettings.requireApprovalForBankCreation,
                  requireApprovalForVouchers:
                    notifSettings.requireApprovalForVouchers,
                  allowWorkersManualJournals:
                    notifSettings.allowWorkersManualJournals ??
                    prev.notifications.allowWorkersManualJournals,
                }
              : prev.notifications,
          }));
        }
        ...
        roles: {
          ...
          [roleKey]: {
            ...
            permissions: Object.fromEntries(
              Object.keys(prev.rbac.roles[roleKey].permissions).map((k) => [
                k,
                true,
              ]),
            ),
  ...
  return (
    <PermissionGuard isAdminOnly={true}>
      <>
        <Navbar title="إعدادات النظام" />
        <div
          className="flex-1 p-4 md:p-6 bg-slate-50/40 dark:bg-transparent min-h-screen"
          dir="rtl"
        >
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 rounded-2xl bg-gradient-to-l from-primary/10 via-primary/5 to-transparent border border-primary/20 p-5 shadow-sm mb-6">
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-1">
                  تحكم كامل في سلوك النظام والعلامة التجارية
                </p>
              </div>
            ...
            <div className="flex-1 space-y-5 min-w-0">
              ...
              {tab === "company" && (
                <>
                  <Card title="بيانات الشركة" icon={Building2}>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <Field
                        ...
                        hint="يظهر في التقارير والفواتير"
                      >
                        <Input
                      ...
                      <Field
                        ...
                        hint="Company name in English"
                      >
                        <Input
                          value={s.company.nameEn}
                          onChange={(v) => update("company", "nameEn", v)}
                  ...
                  <Card title="صورة الشركة والختم والباركود" icon={ImageIcon}>
                    ...
                    <div className="flex flex-wrap gap-4 pt-2">
                      <Toggle
                        ...
                        onChange={(v) =>
                          update("invoice", "showLogoOnPrint", v)
                        }
                      ...
                      <Toggle
                        ...
                        onChange={(v) =>
                          update("invoice", "showStampOnPrint", v)
                        }
                      ...
                      <Toggle
                        ...
                        onChange={(v) =>
                          update("invoice", "showBarcodeOnPrint", v)
                        }
                  ...
                  <Card title="ترقيم الفواتير" icon={Hash}>
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                      <Field
                        ...
                        hint="بادئة رقم الفاتورة"
                      >
                        <Input
                          value={s.invoice.salesPrefix}
                          onChange={(v) => update("invoice", "salesPrefix", v)}
                          placeholder="INV"
                          dir="ltr"
                        />
                      </Field>
                      <Field label="بادئة فواتير المشتريات">
                        <Input
                          value={s.invoice.purchasePrefix}
                          onChange={(v) =>
                            update("invoice", "purchasePrefix", v)
                          }
                          placeholder="PUR"
                          dir="ltr"
                        />
                      ...
                      <Field label="بادئة عروض الأسعار">
                        <Input
                          value={s.invoice.quotationPrefix}
                          onChange={(v) =>
                            update("invoice", "quotationPrefix", v)
                          }
                          placeholder="QUO"
                          dir="ltr"
                        />
                      ...
                      <Field
                        label="مسمى فواتير البيع"
                        hint="يظهر كعنوان للفاتورة"
                      >
                        <Input
                          value={s.invoice.salesInvoiceName}
                          onChange={(v) =>
                            update("invoice", "salesInvoiceName", v)
                          }
                          placeholder="فاتورة مبيعات"
                        />
                      </Field>
                      <Field
                        label="مسمى فواتير الشراء"
                        hint="يظهر كعنوان للفاتورة"
                      >
                        <Input
                          value={s.invoice.purchaseInvoiceName}
                          onChange={(v) =>
                            update("invoice", "purchaseInvoiceName", v)
                          }
                      ...
                      <Field label="رقم البداية" hint="أول رقم تسلسلي">
                        <Input
                          value={s.invoice.startingNumber}
                          onChange={(v) =>
                            update("invoice", "startingNumber", Number(v))
                          }
                          type="number"
                          placeholder="1"
                        />
                    ...
                    <div className="bg-slate-50 dark:bg-slate-800/50 rounded-xl p-4 space-y-3">
                      <p className="text-xs font-bold text-slate-500 uppercase tracking-wider flex items-center gap-2">
                        <Eye className="w-3.5 h-3.5" /> معاينة أرقام الفواتير
                        القادمة
                      </p>
                      <div className="flex flex-wrap gap-6 text-sm">
                        <div className="flex flex-col gap-1">
                          ...
                          <strong className="font-mono text-primary bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 px-3 py-1.5 rounded-lg shadow-sm">
                            {s.invoice.salesPrefix}-
                            {String(s.invoice.startingNumber).padStart(4, "0")}
                          </strong>
                        </div>
                        <div className="flex flex-col gap-1">
                          <span className="text-[10px] text-slate-400 font-bold uppercase">
                  ...
                  <Card title="ملاحظات أسفل الفاتورة" icon={ScrollText}>
                    <Field
                      label="تظهر في أسفل الفاتورة جهة اليمين"
                      hint="مثل أرقام الموبايل، الموقع، روابط التواصل الاجتماعي"
                    >
                      <textarea
                  ...
                  <Card title="الشروط والأحكام" icon={ScrollText}>
                    <Field
                      label="تظهر أسفل الفاتورة المطبوعة"
                      hint="كل سطر يظهر كنقطة منفصلة"
                    >
                      <textarea
              ...
              {tab === "tax" && (
                <>
                  <Card
                    ...
                    badge={
                      <span
                        className={`text-xs px-2 py-1 rounded-full font-semibold ${s.tax.enabled ? "bg-emerald-100 text-emerald-700" : "bg-slate-200 text-slate-500"}`}
                      >
                    ...
                    {s.tax.enabled && (
                      <div className="space-y-4 pt-2 border-t border-slate-100 dark:border-slate-800 mt-2">
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                          <Field
                            label="اسم الضريبة"
                            hint="مثال: ضريبة القيمة المضافة"
                          >
                            <Input
                              value={s.tax.name}
                              onChange={(v) => update("tax", "name", v)}
                          ...
                          <div className="grid grid-cols-2 gap-3">
                            ...
                            >
                              ...
                              <p className="text-xs text-slate-500 mt-1">
                                الضريبة تُضاف فوق السعر
                              </p>
                              <p className="text-xs font-mono text-primary mt-1">
                                سعر {s.tax.rate > 0 ? `+ ${s.tax.rate}%` : ""}
                              </p>
                            </button>
                            <button
                              type="button"
                            ...
                            >
                              ...
                              <p className="text-xs text-slate-500 mt-1">
                                الضريبة مدمجة في السعر
                              </p>
                              <p className="text-xs font-mono text-primary mt-1">
                                السعر يشمل{" "}
                                {s.tax.rate > 0 ? `${s.tax.rate}%` : "الضريبة"}
                              </p>
                            </button>
                          </div>
                        </div>
                  ...
                  <Card title="إعدادات العملة والأرقام" icon={Landmark}>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      ...
                      <Field
                        label="الدقة العشرية"
                        hint="عدد الخانات بعد الفاصلة"
                      >
                        <Select
                          value={s.inventory.decimalPrecision}
                          onChange={(v) =>
                            update("inventory", "decimalPrecision", Number(v))
                          }
                          options={[
                    ...
                    <div className="mt-4 p-4 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-100 dark:border-slate-800">
                      ...
                      <div className="flex items-baseline gap-2">
                        ...
                        <span className="text-sm font-bold text-slate-500">
                          {s.inventory.currency}
                        </span>
                      </div>
                      <p className="text-[10px] text-slate-400 mt-1 italic">
                        مثال لمبلغ تجريبي متأثر بالدقة العشرية
                      </p>
                    </div>
                  </Card>
                </>
                ...
                <Card title="إعدادات المخزون والخزينة" icon={Package}>
                  <div className="space-y-1">
                    <Toggle
                      ...
                      onChange={(v) =>
                        update("inventory", "allowNegativeStock", v)
                      }
                  ...
                  <div className="pt-3 border-t border-slate-100 dark:border-slate-800 space-y-4">
                    <Toggle
                      ...
                      onChange={(v) =>
                        update("inventory", "lowStockAlertEnabled", v)
                      }
                    ...
                    {(s.inventory.lowStockAlertEnabled ?? true) && (
                      <Field
                        label="الحد الأدنى الافتراضي للمخزون المنخفض"
                        hint="يُطبَّق على الأصناف التي لم يُحدَّد لها حد أدنى خاص"
                      >
                  ...
                  {s.inventory.allowNegativeStock && (
                    <div className="flex items-start gap-3 p-4 bg-blue-50 dark:bg-blue-900/10 border border-blue-200 dark:border-blue-800 rounded-xl">
                      ...
                      <p className="text-sm text-blue-700 dark:text-blue-400">
                        ملاحظة: عند البيع بدون رصيد، سيتم إتمام الفاتورة وسيظهر
                        للمستخدم تنبيه بأن الصنف لا يوجد له مخزون كافٍ.
                      </p>
                    </div>
                  )}
                </Card>
                ...
                <Card title="إعدادات التنبيهات" icon={Bell}>
                  <div className="space-y-1">
                    <Toggle
                      ...
                      onChange={(v) =>
                        update("notifications", "staffActivityAlerts", v)
                      }
                    ...
                    <Toggle
                      ...
                      onChange={(v) =>
                        update("notifications", "inventoryAlerts", v)
                      }
                    ...
                    <Toggle
                      ...
                      onChange={(v) =>
                        update("notifications", "vaultBankAlerts", v)
                      }
                    ...
                    {s.notifications.vaultBankAlerts && (
                      <div className="px-3 pb-3">
                        <Field
                          ...
                          hint="سيتم التنبيه عند وصول الرصيد لهذا المبلغ أو أقل"
                        >
                          <div className="flex items-center gap-3">
                            <Input
                    ...
                    <Toggle
                      ...
                      onChange={(v) =>
                        update("notifications", "financialAlerts", v)
                      }
                    ...
                    <Toggle
                      ...
                      onChange={(v) =>
                        update("notifications", "showDueDateOnInvoices", v)
                      }
              ...
              {tab === "rbac" && (
                <div className="space-y-6">
                  <Card
                    title="إعدادات الموافقة (Approval Workflows)"
                    icon={ShieldCheck}
                  >
                    <div className="space-y-1">
                      <Toggle
                      ...
                      <div className="pt-3 border-t border-slate-100 dark:border-slate-800" />
                      <Toggle
                        checked={s.notifications.allowWorkersManualJournals}
                        onChange={(v) =>
                          update(
                            "notifications",
                            "allowWorkersManualJournals",
                        ...
                        label="السماح للموظفين بإضافة قيود يدوية"
                        description="يسمح للموظفين بعمل قيود يدوية بدون الحاجة لتفعيل وضع الإدارة"
                      />
                    </div>
                  </Card>
                      ...
                      <table className="w-full text-sm">
                        <thead>
                          <tr className="bg-slate-50 dark:bg-slate-800/60 border-b border-slate-200 dark:border-slate-700">
                            <th className="px-4 py-3 text-right text-xs font-bold text-slate-500 uppercase w-48">
                              الصلاحية
                            </th>
                            {roleKeys.map((rk) => (
                              <th key={rk} className="px-4 py-3 text-center">
                                <div className="space-y-1">
                                  ...
                                  <div className="flex gap-1 justify-center">
                                    <button
                                      onClick={() => grantAll(rk)}
                                      className="text-[10px] px-1.5 py-0.5 bg-emerald-100 text-emerald-700 rounded hover:bg-emerald-200 transition-colors"
                                      title="منح الكل"
                                    >
                                      <Eye className="w-2.5 h-2.5 inline" /> كل
                                    </button>
                                    <button
                                      onClick={() => revokeAll(rk)}
                                      className="text-[10px] px-1.5 py-0.5 bg-rose-100 text-rose-700 rounded hover:bg-rose-200 transition-colors"
                                      title="سحب الكل"
                                    >
                            ...
                            <React.Fragment key={group.group}>
                              ...
                              {group.perms.map((perm) => (
                                ...
                                >
                                  ...
                                  {roleKeys.map((rk) => {
                                    const checked =
                                      !!s.rbac.roles[rk]?.permissions[perm.key];
                                    return (
                                      <td
                                        key={rk}
                                      ...
                                      >
                                        <button
                                          type="button"
                                          onClick={() =>
                                            updatePerm(rk, perm.key, !checked)
                                          }
                        ...
                        <table className="w-full text-sm text-right">
                          <thead>
                            <tr className="bg-slate-50 dark:bg-slate-800/60 border-b border-slate-100 dark:border-slate-800">
                              <th className="px-5 py-3 font-bold text-slate-700 dark:text-slate-300">
                                المستخدم
                              </th>
                              <th className="px-5 py-3 font-bold text-slate-700 dark:text-slate-300">
                                الصلاحية
                              </th>
                              <th className="px-5 py-3 font-bold text-slate-700 dark:text-slate-300 text-center">
                                الإجراءات
                              </th>
                            </tr>
                          </thead>
        ...
        >
          <AlertDialogContent
            className="sm:max-w-[440px] border-0 shadow-2xl rounded-[2.5rem] p-0 overflow-hidden bg-white dark:bg-slate-900"
            dir="rtl"
          >
