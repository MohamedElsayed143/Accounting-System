...
function LedgerContent() {
  ...
  const isSpecializedAccount = !!(
    selectedAccount?.treasurySafe || selectedAccount?.treasuryBank
  );
  const isEntityAccount = !!(
    selectedAccount?.customer || selectedAccount?.supplier
  );
  ...
  return (
    <PermissionGuard permissionKey="accounting_ledger_view">
      <>
        ...
        <div
          className="flex-1 p-4 md:p-8 space-y-8 bg-slate-50/30 dark:bg-transparent min-h-screen"
          dir="rtl"
        >
          {/* Header Section */}
              ...
              {summaryItems.map((item, idx) => (
                ...
                >
                  <div className="flex items-center justify-between mb-3">
                    <div
                      className={cn(
                        "p-2.5 rounded-xl bg-opacity-10 dark:bg-opacity-20",
                        item.bg,
                      )}
                    >
            ...
            <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] border-2 border-dashed border-slate-100 dark:border-slate-800 py-32 flex flex-col items-center justify-center gap-6 text-center shadow-inner print:hidden">
              ...
              <div className="space-y-2">
                <h3 className="text-2xl font-black text-slate-900 dark:text-white">
                  بانتظار اختيار الحساب
                </h3>
                <p className="text-slate-500 text-sm max-w-sm mx-auto font-medium">
                  قم باختيار حساب من شجرة الحسابات لعرض دفتر الأستاذ التفصيلي
                  والحركات المالية
                </p>
              </div>
...
export default function LedgerPage() {
  return (
    <Suspense fallback={<div className="flex-1 p-8 text-center font-bold text-slate-500 animate-pulse">جاري التحميل...</div>}>
      <LedgerContent />
    </Suspense>
  );
}
