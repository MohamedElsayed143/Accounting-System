...
import { JournalPrintableVoucher } from "@/components/accounting/JournalPrintableVoucher";
...
export function JournalList({ 
  ...
  React.useEffect(() => {
    if (printingEntry) {
      // Small delay to ensure React has finished rendering the printable component
      const timer = setTimeout(() => {
        window.print();
        // Give the browser time to initiate the print process before removing the element
        setTimeout(() => setPrintingEntry(null), 500);
      }, 300);
  ...
  const filteredEntries = initialEntries.filter(entry => {
    ...
    const matchesDate = isDateInFilter(entry.date, dateFilter);
  ...
  return (
    <>
      {/* Main UI Container - Hidden during print */}
      <div className="space-y-8 print:hidden" dir="rtl">
        {/* Header Section */}
                    ...
                    <div className="flex items-center gap-8">
                      ...
                      <div className="flex items-center gap-2">
                        <Button 
                          variant="ghost" 
                          size="icon" 
                          className="h-12 w-12 rounded-2xl bg-slate-50 dark:bg-slate-800 text-slate-400 hover:text-primary hover:bg-primary/10 transition-all"
      ...
      {/* Printable Components (Hidden on Screen, Visible during Print) */}
      {printingEntry && (
        <JournalPrintableVoucher 
          ref={printVoucherRef} 
          entry={printingEntry} 
