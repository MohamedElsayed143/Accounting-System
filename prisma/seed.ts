...
// Removed global prisma client

export async function seedTenantData(tx: any, tenantSchema: string) {
  await tx.$executeRawUnsafe(`SET search_path TO "${tenantSchema}"`);
  console.log('🌱 بدء إنشاء البيانات الافتراضية...')

  // 1. الخزنة الرئيسية (Treasury Safe)
  const safe = await tx.treasurySafe.upsert({
    where: { id: 1 },
    update: {},
    create: {
      id: 1,
  ...
  // --- المستوى الأول (L1) ---
  const assets = await tx.account.upsert({
    where: { code: '1' },
  ...
  const fixedAssets = await tx.account.upsert({
    where: { code: '11' },
  ...
  const currentAssets = await tx.account.upsert({
    where: { code: '12' },
  ...
  const cashGroup = await tx.account.upsert({
    where: { code: '1201' },
  ...
  const mainCashAccount = await tx.account.upsert({
    where: { code: '120101' },
    update: {
  ...
  const currentLiabilities = await tx.account.upsert({
    where: { code: '21' },
  ...
  const longTermLiabilities = await tx.account.upsert({
    where: { code: '22' },
  ...
  const capitalL2 = await tx.account.upsert({
    where: { code: '31' },
  ...
  const retainedL2 = await tx.account.upsert({
    where: { code: '32' },
  ...
  const reservesL2 = await tx.account.upsert({
    where: { code: '33' },
  ...
  const dividendsL2 = await tx.account.upsert({
    where: { code: '34' },
  ...
  const operatingRevenue = await tx.account.upsert({
    where: { code: '41' },
  ...
  const nonOperatingRevenue = await tx.account.upsert({
    where: { code: '42' },
  ...
  // 51: مصروفات تشغيلية
  const operatingExpenses = await tx.account.upsert({
    where: { code: '51' },
  ...
  const adminExpenses = await tx.account.upsert({
    where: { code: '52' },
  ...
  const sellingExpenses = await tx.account.upsert({
    where: { code: '53' },
  ...
  // 54: أجور ورواتب
  const wagesExpenses = await tx.account.upsert({
    where: { code: '54' },
  ...
  const costs = await tx.account.upsert({
    where: { code: '6' },
    update: { level: 1, isTerminal: false, isSelectable: false },
  ...
  const directPurchaseCosts = await tx.account.upsert({
    where: { code: '61' },
  ...
  // 6104: المشتريات
  const purchasesGroup = await tx.account.upsert({
    where: { code: '6104' },
  ...
  const operationalProductCosts = await tx.account.upsert({
    where: { code: '62' },
  ...
  const damagedGoodsGroup = await tx.account.upsert({
    where: { code: '6204' },
  ...
  const inventoryAccount = await tx.account.findUnique({ where: { code: '1203' } });
  if (inventoryAccount) {
    await tx.account.upsert({
      where: { code: '120301' },
...
if (require.main === module || (process.argv[1] && process.argv[1].endsWith('seed.ts'))) {
  ...
  prisma.$transaction(async (tx: any) => {
    await seedTenantData(tx, 'public');
  }, {
